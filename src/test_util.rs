...
/// Because every test gets a distinct number, no sibling test is chasing the same port when the
/// agent finally binds it; the only residual race is an unrelated external process grabbing the
/// port in a microsecond window, which the next candidate handles.
pub(crate) fn alloc_port() -> u16 {
    const LO: u32 = 20_000;
    const SPAN: u32 = 40_000; // 20000..60000
