...
- Lib tests at HEAD (2026-06-21): `mycelium --lib` default ≈ **287**, split across the two
  crates — `mycelium-core` (**127**, the Layers I+II substrate, incl. the WS-B M11 codec /
  `serde_fixint` / Merkle-digest suites) + `mycelium` (the upper layers; the v2-M3 handle pushdown
  put the `SchemaHandle`/`KvHandle`/`MeshHandle` pure tests in core, the `GossipAgent`-driven cases
  in `src/agent/{schema,kv,mesh}_handle_tests.rs`). `--features tls` / `compliance` add the WS1 RBAC
  + WS2 audit + WS4 OIDC + WS5 rotation suites (the core `sys/` tripwire and WS3 crown-jewel
  data-at-rest/egress tests are feature-free, already in the default count). Companion crates:
  `mycelium-tuple-space` **31**, `mycelium-blackboard` **15**, `mycelium-agentfacts` **9**.
  **These counts grow every PR — run `cargo test --lib` (and `--features tls,metrics,a2a,llm` /
  `compliance`) for the live total; don't trust a memorised number** (this bullet has drifted
  twice). Clippy `-D warnings` clean across crates. The codec equivalence tests use `bincode` as a
  dev-dependency oracle only — not in the shipped tree.
