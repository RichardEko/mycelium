...
| [`mycelium-wiki.md`](mycelium-wiki.md) | 📋 **Proposed — design-complete, demand-gated** (2026-07-02) — a **group-scoped LLM-maintained wiki** as a fourth public-API coordination primitive (durable/curated, alongside tuple-space's pull and blackboard's claim); the long-term-memory sibling of the blackboard's working memory. **Phase 0 is done ahead of build:** the design record [`wiki-concurrent-edit.md`](../design/wiki-concurrent-edit.md) pins the section-addressing + merge unit, the merge semantics (section-granular LWW + a recallable serialised "curator"; free-for-all LLM merge rejected-with-evidence — doesn't converge), the curator state machine, the Capability/Skill/Group identity model ("competence is a capability, knowledge is not"), and the full `lib.rs` API + `WikiConfig` surface. Public-API-only companion crate; **gated on demand, not feasibility** — the sketch is ready to promote to a phased build the moment a long-lived fleet visibly re-derives the same knowledge. Awaiting that signal. |
