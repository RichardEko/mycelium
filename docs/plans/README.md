...
| [`mycelium-wiki.md`](mycelium-wiki.md) | 📋 **Proposed** (2026-07-02) — a **group-scoped LLM-maintained wiki** as a fourth public-API coordination primitive (durable/curated, alongside tuple-space's pull and blackboard's claim). The long-term-memory sibling of the blackboard's working memory. Load-bearing design problem: concurrent prose edits don't LWW-merge → section-granular keys + a recallable "curator" role (LLM reconcile serialised at one writer-of-record; free-for-all LLM merge rejected — doesn't converge). Public-API-only companion crate; **gated on demand, not feasibility** — build when a long-lived fleet visibly re-derives the same knowledge. Awaiting go-ahead. |
