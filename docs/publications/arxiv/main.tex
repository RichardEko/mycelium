...
\item \textbf{Opacity Backpressure.} A saturated node writes an opacity flag to
  \texttt{sys/load/\{self\}/\{kind\}} (TTL~$\approx$5\,s, where \texttt{kind} is
  the application-defined opacity category), making itself invisible to resolvers
  at the next stage. The topology self-throttles without any flow-control protocol.
...
by integration scenario~11 (repository and test harness: \S8).
...
Mycelium's correctness across its three-layer architecture is validated by 304
...
\url{https://github.com/RichardEko/mycelium} (commit
\texttt{1717d4e}, tag \texttt{paper-submission-v1}) under the AGPL-3.0 licence.
...
correlation key---a keyed-exact-match \texttt{take} would cover this without
...
\item \textbf{The three-arm work-distribution experiment} on one substrate:
  broker-mediated prediction, locally evaluated prediction, and pull (tuple
  space), under heterogeneous and drifting worker service times. Because pull
  abolishes the staleness/misroute vocabulary, the common metrics must be
  outcome-level---end-to-end job latency, throughput, idle-while-work-exists
  time, and fairness as worker heterogeneity grows. The companion paper's
  framework predicts the prediction arms recover only as workers are made
  clone-like and synchronous; the binaries and the crate for all three arms
...
\item \textbf{A blackboard companion as the next constructive test}---rebuild
  opportunistic shared-working-memory coordination (the blackboard of
  Section~3.1, reborn as LLM-agent scratchpads) on the public API: facts as KV
  writes, trigger predicates as boundaries, plus one new primitive---competitive
  destructive claim-by-predicate with the tuple space's WAL/in-flight
  discipline. Together with a keyed-exact-match \texttt{take} for fan-in
  joins, this would extend the Section~8 argument from pipeline workloads to
  the associative-claiming workloads at the lane model's boundary, on the same
  no-coordinator terms.
\end{itemize}
