...
\begin{abstract}
The companion papers argue that coordinator-based architectures fail
structurally~\cite{nicholson2026} and that coordinator-free, locally-resolved
substrate design is the \emph{epistemically privileged} response across four
independent domains---homologous rather than identical---for operations whose
correctness depends on current local state~\cite{nicholson2026hlks}. This paper argues that epistemic
...
Symmetry, and Exit---that capture resistance requires in addition to epistemic
correctness, and argue, following the cross-domain method of the
...
computing substrates. We apply the MCB/P/S evaluative framework (Moral Circle
Breadth, Polycentricity, Surplus-Claim Regime), introduced for monetary architectures
in~\cite{nicholson2025}, as a scorecard for capture resistance---the dimension it was
designed to measure. We then confront the hardest question the
...
inherits a worthless network. On this reading Bitcoin is evidence that capture
resistance can be emergent and robust; the sharper open question is whether a
\emph{computing} substrate---which
...
The companion paper~\cite{nicholson2026hlks} argues that across economics,
governance, organisations, and distributed computing, coordinator-free
locally-resolved substrate design is the \emph{epistemically privileged} response to
the problem of heterogeneous local knowledge---homologous across the four domains
rather than identical, and scoped to operations whose correctness turns on current
local state. Four substrate correctness properties---local signal propagation,
emergent group formation, minimal federation invariants, and subsidiarity-respecting
consensus---are motivated by the (informal) Coordinator Trap Theorem and appear,
independently derived, in each domain.
...
\item Defines four further substrate properties---Capture Resistance, Mandate TTL,
  Epistemic Symmetry, and Exit---and argues that capture resistance calls for them in
  addition to epistemic correctness (\S\ref{sec:properties}).
\item Applies the MCB/P/S framework---introduced for monetary architectures
  in~\cite{nicholson2025}---as a capture-resistance scorecard, arguing that the three
  axes track the distinctions the capture-resistance properties are designed to
  protect (\S\ref{sec:mcbps}).
...
\paragraph{A note on evidence.} Unlike its companions---which carry a latency
measurement and a constructive pull instantiation~\cite{nicholson2026hlks}, and a
worked scoring example~\cite{nicholson2025}---this paper is structural. Its
contribution is conceptual and cross-domain; it offers no new measurement, and the
empirical and constructive grounding of capture resistance in the computing domain
(\S\ref{sec:domains}, \S\ref{sec:hard}) is, we acknowledge, the least developed part
of the research programme. The properties below are design desiderata argued to
recur, not quantities measured; the honest accounting of what that leaves open is
\S\ref{sec:open}.
...
cheap---is real and is a central subject of this paper. Like the measures in the
companion frameworks (the heterogeneity measure $H$ of~\cite{nicholson2026hlks}, the
illustrative scores of~\cite{nicholson2025}), $C_{\text{capture}}$ is here a
qualitative desideratum, not an operationalised metric. What is concrete is the
comparative judgement it licenses: a substrate with no engineered or emergent
cost-of-capture is more exposed than one whose cost is high and rises with the stake.
...
paper~\cite{nicholson2026hlks} to score epistemic distinctions. As that paper notes,
the framework belongs most naturally to the analysis of capture, which this paper
develops.
...
Capture Resistance, Mandate TTL, Epistemic Symmetry, and Exit \emph{tends} to score
broad-MCB, high-P, distributed-surplus, and the failure of a property tends to show
as degradation on a corresponding axis---a directional relationship, not an
entailment, in the same spirit as the $P \rightsquigarrow S$ tendency
of~\cite{nicholson2025}. Table~\ref{tab:propaxis} states the \emph{primary} mappings;
they are indicative rather than one-to-one---Exit, for instance, bears on both
Surplus-Claim and Moral Circle Breadth.
...
\bottomrule
\end{tabularx}
\caption{Each capture-resistance property \emph{primarily} protects one MCB/P/S axis;
the mapping is indicative rather than one-to-one, and a property's failure tends to
show as degradation on that axis. The MCB/P/S framework thus serves as the
\emph{scorecard} for the properties rather than an independent construct.}
\label{tab:propaxis}
\end{table}
...
capture in the abstract---Bitcoin indicates the properties can deliver it---but whether a
...
extract---but Bitcoin suggests these are strongest when an aligned value signal stands
