...
The implementation is publicly available at https://github.com/RichardEko/mycelium (commit `1717d4e`, tag `paper-submission-v1`) under the AGPL-3.0 licence. The integration test harness is included in the repository and reproducible with a single `make test` invocation against a Docker-composed 5-node cluster. The AGPL-3.0 licence is intentional: applications that deploy Mycelium as part of a networked service must publish their source under the same terms, closing the SaaS loophole that permissive licences leave open. Organisations requiring proprietary embedding may obtain a commercial licence from Tathata Systems Ltd.
...
- **The three-arm work-distribution experiment** on one substrate: broker-mediated prediction, locally evaluated prediction, and pull (tuple space), under heterogeneous and drifting worker service times. Because pull abolishes the staleness/misroute vocabulary, the common metrics must be outcome-level — end-to-end job latency, throughput, idle-while-work-exists time, and fairness as worker heterogeneity grows. The companion paper's framework predicts the prediction arms recover only as workers are made clone-like and synchronous; the binaries and the crate for all three arms are in the repository.
...
- **A blackboard companion as the next constructive test** — rebuild opportunistic shared-working-memory coordination (the blackboard of §3.1, reborn as LLM-agent scratchpads) on the public API: facts as KV writes, trigger predicates as boundaries, plus one new primitive — competitive destructive claim-by-predicate with the tuple space's WAL/in-flight discipline. Together with a keyed-exact-match `take` for fan-in joins, this would extend the §8 argument from pipeline workloads to the associative-claiming workloads at the lane model's boundary (§9.4), on the same no-coordinator terms.
