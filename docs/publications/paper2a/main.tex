...
not identical.'' This paper argues that the parallel is structurally
\emph{homologous}---the domains instantiate a common coordination problem under
specified assumptions---and supplies the formal framework that makes the
correspondence precise. We introduce
...
while locally evaluated mechanisms reduce that error and self-resolved \emph{pull}
eliminates the specific prediction variable in which it is expressed. We then identify four
...
traditions. A short note relates the substrate's
distributive dimension to the MCB/P/S framework~\cite{nicholson2025}, deferring its
development to a companion paper on capture. We distinguish two grades of local
resolution---locally evaluated
...
the substrate's public primitives---whose test suite verifies exactly-once-effect
...
This paper argues that the parallel is structurally \emph{homologous}: under an
appropriate formal abstraction, the four traditions are addressing a common
coordination problem, not merely analogous problems, and their prescriptions
converge for that reason. We do not claim a proof of formal identity---``exact''
would overstate what an informal cross-domain argument can establish
(\S\ref{sec:scope})---but the qualification in~\cite{nicholson2026} (``suggestive
rather than exact'') can be \emph{sharpened}: the resemblance is not loose analogy
but a shared problem-class under specified HLKS assumptions, within which one class
of solution is epistemically privileged. The qualification in~\cite{nicholson2026}
is correct as an honest statement of what that paper had established---a
single-domain architectural argument with two cross-domain observations in its
Discussion---and the abstraction below makes the correspondence precise enough to
say in what respect, and under what conditions, it holds.
...
\item A formal abstraction---the \emph{Heterogeneous Local Knowledge System}
  ...
  communication latency between agents, and a correctness relation specifying when
  an action is adequate relative to the current joint local state (rather than a
  planner-computable optimum, which several domains deny exists).
...
\item An informal statement of the \emph{Coordinator Trap Theorem}, with a
  ...
  state change, while locally evaluated mechanisms reduce it and self-resolved pull
  eliminates the prediction variable in which it is expressed.
...
\item A short note relating the substrate's \emph{distributive} dimension---who is
  first-class, how dispersed decision rights are, who captures coordination
  surplus---to the MCB/P/S framework~\cite{nicholson2025}, with the full normative
  and capture-resistance treatment deferred to a companion paper. This paper's claim
  is confined to \emph{epistemic} correctness.
...
epistemically privileged architecture for the operations whose correctness depends
on current heterogeneous local state, and the convergent prescriptions of the four
traditions are evidence supporting this claim rather than independent coincidences.

\subsection{Scope of the Claim}
\label{sec:scope}

Because the argument spans four traditions, it is worth stating precisely what is,
and is not, being claimed. We claim that four traditions, reasoning independently
from different premises and with different formal apparatus, converge on the same
\emph{structural} constraint on substrate design. The evidence is the
\emph{independence} of the derivations, not the resemblance of the domains:
convergent derivation, as in convergent evolution, is stronger than analogy
precisely because nothing is transferred between the cases. That the same
structure recurs in domains as different as price formation, commons governance,
organisational adaptation, and distributed agent coordination is not an
embarrassment to be minimised but the phenomenon itself---the signature of a
complex adaptive system, in which the organising constraint lies in the structure
of the problem, not the substance of the domain.

We do not claim that the domains are identical---they are not, and the companion
paper's qualification on that point stands. We do not claim that any domain's
specific \emph{mechanisms} carry across: the substrate of the computing domain
implements neither a price mechanism nor a nested governance rule; it exhibits the
same coordinator-free \emph{property} that economics and governance independently
identify as correct. The pattern recurs; the mechanisms do not transfer. Nor does
the structural argument depend on endorsing the normative commitments of any source
framework---the ethical content of Moral Circle Breadth, for instance, is
orthogonal to whether that axis marks a structural distinction between substrate
designs, and we adopt these frameworks for their structural content alone. Finally,
the engineering validity of the computing instantiation (\S\ref{sec:empirical}, and
the companion paper) is established within the computing domain on its own terms; it
does not depend on a reader accepting the cross-domain convergence thesis argued
here.

We state the central result as an informal theorem argued with care
(\S\ref{sec:formalstatus}), not as a proved one; its formal status in each domain is
discussed there and revisited among the open questions (\S\ref{sec:open}).
...
\item $\lambda = \{\lambda_{ij}\}_{i,j \in A \cup \{C\}}$ is the family of
  ...
  present).
\item $R \subseteq \bigl(\prod_i S_i\bigr) \times \mathit{Actions}$ is a
  \emph{correctness relation}: it specifies when an action is adequate
  \emph{relative to the current joint local state}. We deliberately do not posit an
  \emph{optimal decision function} computable from a global summary---in several
  domains, notably economics, no such function is available to any central party,
  which is part of the problem rather than an assumption we may help ourselves to.
  $R$ encodes only that, for the operations at issue, correctness depends on current
  local states and is not recoverable from a globally available sufficient statistic.
\end{itemize}
\end{definition}

The definition is deliberately spare. We do not specify the structure of the
state spaces beyond their heterogeneity, the precise form of $R$ beyond its
dependence on current local states, or the topology of the communication
...
A coordinator $C$ attempting to choose an action satisfying $R$ from a position
outside the agent population can only observe estimates $\hat{K}_i(t) = K_i(t - \lambda_{iC})$:
...
Consider an HLKS in which
\begin{enumerate}[label=(\roman*)]
\item local states are heterogeneous across agents---differing in structure,
  content, dynamics, or decision relevance---and not fully derivable from one another
  or from a common global summary (it is \emph{not} required that the state spaces
  $S_i$ and $S_j$ be non-isomorphic: agents sharing a schema but differing materially
  in values, capabilities, or histories still satisfy this),
\item state-change rates $\delta_i$ are bounded away from zero, and
\item a correct decision depends on the agents' current local states in a way not
  recoverable from any globally available summary---no sufficient statistic
  substitutes for the current joint state, and the dependence is not separable into
  independent per-agent terms.
\end{enumerate}
Then, for a coordinator acting on an estimate $\hat{K}_i(t)$ of each agent's state
formed at communication latency $\lambda_{iC} > 0$:
\begin{enumerate}
\item The per-agent epistemic error
  $\varepsilon_i(t) = \mathrm{dist}(K_i(t), \hat{K}_i(t))$ is positive \emph{in
  expectation}, $\mathbb{E}[\varepsilon_i(t)] > 0$, whenever $K_i$ varies over the
  interval $\lambda_{iC}$. (It may be momentarily zero---when a state happens not to
  change, or changes and returns---so the claim is on expected, not strict
  pointwise, error.)
\item Under~(iii), the expected aggregate error
  $\mathbb{E}\!\left[\sum_i \varepsilon_i(t)\right]$ is non-decreasing in the number
  of \emph{relevant} agents $n$ (those the decision depends on), in a measure $H$ of
  state-space heterogeneity, and in the mean state-change rate $\bar{\delta}$.
  Without~(iii)---a separable decision, or one admitting a sufficient statistic, or
  agents the decision ignores---this need not hold.
\item No increase in coordinator computational capacity reduces
  $\mathbb{E}[\varepsilon_i]$ below the bound imposed by $\lambda_{iC}$. The
  constraint is epistemic, not computational: faster computers do not help.
\end{enumerate}
\end{informaltheorem}
...
measure $H$ (which a domain-specific version would instantiate differently---
type and capability variance in compute, preference, resource and context
diversity in economics, institutional and contextual diversity in governance,
task, customer and technical variance in organisations); and a specific decision
criterion under which a correct action degrades with stale inputs. Each commitment is sensible for any
...
A further subtlety, sharpest in economics, is that the theorem must not assume a
planner-\emph{computable} optimal decision function: Hayek's argument is precisely
that no such function is available to a central planner in the first place. We
therefore state condition~(iii) without it---requiring only that a \emph{correct}
action depend on current local states not recoverable from any global summary, which
is weaker than positing a computable global optimum and is what each domain actually
exhibits. The earlier-draft formulation in terms of ``the optimal decision function
$D$'' conceded too much; the conditional form is both safer and closer to the
traditions' own claims.
...
If the coordinator cannot reliably choose an action satisfying $R$, the natural
question is whether any mechanism can. We claim that a locally-resolved signal
mechanism avoids the coordinator's epistemic error by construction. Rather than
aggregating state at a central point and choosing globally, each agent
...
guarantee that the action taken is the one an observer with access to current
aggregate state would prescribe. The gain is correctness
...
This is the key inversion. Coordinator-based mechanisms act against a correctness
specification ($R$ over the current joint state) they cannot evaluate. Locally-resolved
mechanisms satisfy a weaker specification ($R$ restricted to local state) they can
...
local knowledge and attempts to act correctly (to satisfy $R$) from those estimates. The planner's
epistemic error is the $\varepsilon_i$ of Theorem~\ref{thm:coordtrap}; the price
mechanism is the locally-resolved alternative. The prescription is conditional, not
universal: where the relevant knowledge is not purely local---externalities, public
goods, monopoly power, incomplete markets---prices alone underdetermine the correct
action and require supplementary coordination. The HLKS claim is scoped to operations
whose correctness turns on dispersed \emph{current local} state, not to all economic
action.
...
finding expressed in governance vocabulary. The prescription is not that local
governance is automatically just or adaptive: locally-emerged institutions can
entrench exclusion, elite capture, parochialism, or coercion. Ostrom's design
principles are themselves the conditions under which local governance \emph{succeeds};
absent them, polycentricity secures the epistemic fit the framework concerns, not a
good outcome as such.
...
than weakened by the variation in formal apparatus across the four traditions. High
autonomy is not always superior: tasks that are safety-critical, compliance-bound, or
that require synchronised capital allocation or crisis response can need central
coordination. Beinhocker's claim, and ours, concerns the operations whose correctness
depends on fast-changing local knowledge, not a blanket preference for
decentralisation.
...
across the structural properties. Coordinators remain appropriate in computing where
the HLKS conditions fail---homogeneous nodes, bounded low-variance state, or tasks
requiring strong consistency---a case the homogenisation corollary
(\S\ref{sec:homogenisation}) makes precise. The claim is not that coordination is
never appropriate, but that it is mis-applied to heterogeneous, fast-changing,
locally-held state.
...
fleets and market economies are instances of a common coordination problem
(heterogeneous local knowledge under non-zero state-change rates), within which
one class of solution---local resolution---is epistemically privileged. This is
correctness \emph{relative to local action}, not a claim of global optimality or
of uniqueness in every respect; \S\ref{sec:scope} and \S\ref{sec:formalstatus}
bound it.

% ─────────────────────────────────────────────────────────────────────────────
\section{Beyond Epistemic Correctness: a Note on the Distributive Axes}
\label{sec:mcbps}

The HLKS argument so far concerns \emph{epistemic} correctness: which substrate
coordinates heterogeneous local knowledge without acting on stale central estimates.
A substrate also has a \emph{distributive} dimension---whose knowledge is treated as
first-class, how dispersed decision rights are, and who captures the coordination
surplus---which this paper deliberately does not evaluate. That dimension is the
subject of the MCB/P/S framework (Moral Circle Breadth, Polycentricity,
Surplus-Claim Regime), introduced for monetary architectures in~\cite{nicholson2025}
and there given its scoring method and its careful structural tendency
$P \rightsquigarrow S$ (low polycentricity \emph{pressures} concentrated
surplus-claim---a tendency, not a law).

We record only the one structural connection that bears on the present argument:
promoting an agent to coordinator both narrows the set of first-class participants
and creates a control point through which coordination value can be extracted---a
broker's transaction fee, an orchestrator's allocation power. Polycentricity is the
structural remedy for both at once, since locally-resolved coordination admits no
such control point by construction. We resist the stronger reading---that low
participation \emph{implies} concentrated surplus-claim---because rent can also arise
through control of implementation, hosting, protocol evolution, or identity even
where no coordinator routes decisions. That is a capture problem in its own right,
and its structural treatment belongs to the companion paper on capture dynamics, not
here.
...
The Coordinator Trap Theorem motivates four structural properties of a correct HLKS
substrate. The first follows directly from the theorem; the others are design
corollaries---strongly suggested by the framework rather than strictly entailed---and
we are explicit below about which is which. We state each as a property, note its
relation to the framework, and identify its independent appearance across all four
...
single one. They are \emph{sufficient at the level of this framework}---an HLKS
substrate satisfying all four resolves the epistemic problem the Coordinator Trap
Theorem identifies---though we do not claim exhaustiveness over a formal space of
mechanisms, which the informal theorem cannot support. They are not, however, sufficient for all the structural
...
\subsection{Emergent Consensus Is Not a Standing Coordinator}

The distinction that matters is temporal and structural: an emergent consensus
structure is convened for a specific operation and dissolves, where a coordinator
stands. It affects only the agents in the specific ballot and only for its duration;
the substrate returns to full local resolution once the operation completes; and no
\emph{standing} entity exists to become a permanent control point---though whether an
emergent mechanism can be \emph{captured} while convened is a capture question, taken
up in the companion paper, not an epistemic one.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Preliminary Evidence: a Computing-Domain Anchor}
\label{sec:empirical}
...
Second---and this is the part that most directly evidences the framework's
strongest claim---a constructive artifact: a pull-based work-distribution layer
...
\input{data/fig_decision_latency.tex}
...
of magnitude higher than gossip. The substrate is not slow; the serialisation
through a single coordinating agent is. We read this as a concrete computing-domain
instance of the latency gradient the Coordinator Trap predicts---not as a measurement
of a general scaling law, which a single run per condition cannot support.

The broker p99 tail at $N{=}10$ and $N{=}40$ is dominated by the $500$\,ms RPC
timeout, and should be read as a separate \emph{failure tail} rather than ordinary
routing latency: it reflects requests that did not complete within the timeout, and
its magnitude is partly an artefact of the chosen timeout value and this broker
implementation, not of the coordinator trap as such. A claim resting on the tail
would be fragile; the result we rely on is the order-of-magnitude gap in \emph{mean}
routing latency between the two paradigms on the same substrate. A fuller breakdown---
timeout rate, median, and mean computed with and without timed-out calls, reported
separately from the tail---would harden the evidence and is the natural next
measurement; the present table does not yet decompose it.
...
\begin{itemize}
\item \textbf{The space decides nothing---but three roles must be
  separated.} \emph{Epistemic coordination} (predicting and assigning on stale
  state) is what the Coordinator Trap is a theorem about, and the space does none of
  it: it holds no model of any worker's state, makes no allocation decision, and
  predicts nothing; allocation \emph{is} the workers' pull operations, and no
  quantity in the system corresponds to ``misroute.'' \emph{State rendezvous}
  (storing and transferring items) the space \emph{does} perform, so it is a capacity
  concern and a failure domain like any shared medium---answered by the emergent
  failover below, not waved away. \emph{Authority and capture} (who controls the
  mechanism and can extract rent from it) is a third question the space does not
  escape merely by deciding nothing: a rendezvous can still become a control point,
  and that is a capture problem, treated in the companion paper, not an epistemic
  one. Removing the epistemic coordinator does not by itself remove every
  coordinator-shaped risk, and we do not claim it does.
...
\item \textbf{Exactness without prediction.} Ten concurrent producers and
  ten concurrent workers exchange 1{,}000 items with \emph{exactly-once delivery
  under concurrent, no-failure consumption}---each item claimed by a single
  owner---and a parked worker receives a direct hand-off when work arrives.
  Allocation precision under pull comes from the claim operation itself, with no
  participant ever holding a view of another's load. Under worker failure the
  guarantee is \emph{at-least-once} delivery (an expired claim re-queues the item)
  with an idempotent \texttt{complete}, yielding the \emph{exactly-once effect}:
  delivery may repeat, but the effect lands once. ``Exactly-once'' is used
  throughout as shorthand for this effect, not for the (impossible) guarantee of
  non-repeating delivery in an asynchronous system.
...
proportional to scale, heterogeneity, and the rate of state change, while
locally evaluated mechanisms reduce that error and self-resolved pull eliminates
the specific prediction variable in which it is expressed. The four traditions
(Hayek, Ostrom, Beinhocker, and the Holland/Nicholson computing line)
converge on Properties 1--4 because they are independently addressing the same
HLKS problem in four different domain vocabularies. The convergence is evidence
for the shared structure; the HLKS framework is the proposed explanation. Neither
closes the argument by itself---independent resemblance is strong evidence, not
proof---but the abstraction makes precise \emph{in what respect} the domains
correspond.
...
single-domain scope of that paper. With the HLKS framework it can be
\emph{sharpened} rather than removed: the resemblance is structurally
\emph{homologous}---the four domains instantiate a common coordination problem
under the HLKS assumptions, and the structural identity of
Table~\ref{tab:identity} states in what respect, mapping each HLKS component to its
form in each domain. What we do not claim is formal identity in a single
parametric model; \S\ref{sec:formalstatus} explains why such a model is neither
available nor necessary for the homology claim.
...
invoking consensus; and, where distributive or capture questions matter, evaluate
the resulting substrate using a separate framework such as MCB/P/S. The same set of
design moves applies whether the substrate is a
...
\emph{precise} with the right abstraction. That observation is the seed of the
present paper.
