...
| [`design/`](design/) | **Decision record / ADR** — *why* a design choice | Point-in-time, cross-cutting design decisions + binding contracts: the [exactly-once-effect contract](design/exactly-once-effect.md) (the discipline the tuple-space + blackboard share), the [OR-Map evaluation](design/or-map-gcap-evaluation.md) ("keep LWW+HLC"), the [group-wiki concurrent-edit record](design/wiki-concurrent-edit.md) (section addressing + curator state machine — pre-build, for the [proposed `mycelium-wiki`](plans/mycelium-wiki.md)), and the [legible-emergence taxonomy](design/legible-emergence-taxonomy.md) (Phase 0 of the diagnosability plan — pathology classification + the `ViewConfidence` reframe). The home for "we evaluated X, chose Y, here's why." |
