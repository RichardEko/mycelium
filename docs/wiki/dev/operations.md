...
| `GET /stats` | `node_id`, optional `cluster_name` (`GOSSIP_CLUSTER_NAME` — also a `cluster` global label on `/metrics` and an AgentFacts field), `store_entries`, `dropped_frames`, `task_count`, and the tripwire counters (`commit_conflicts`, `sys_namespace_violations`, `cap_authz_violations`, `schema_mismatch`, `rate_limited_senders`) |
