...
Sun Microsystems' Java/Jini~\cite{jini-arch,jini-spec} introduced the insight
that distributed resource registrations should \emph{decay} rather than persist
indefinitely. A service holds a lease on its registration; if it does not renew,
the registration expires. This provides implicit failure detection without
requiring an explicit deregistration protocol. The insight is correct. The
implementation was protocol-heavy: explicit \texttt{Lease} objects,
