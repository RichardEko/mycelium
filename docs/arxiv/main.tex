...
runtime left a gap with no mechanism for repair. Paremus Service Fabric
...
measurements. White~\cite{white2025} finds that inference energy per query
varies by two orders of magnitude across model sizes, and that a 17$\times$
energy cost increase between 7B and 72B parameter models yields no statistically
significant accuracy improvement for mathematical code generation. When each
coordinator iteration already carries this energy weight, quadratic accumulation
is not a theoretical concern---it is an operational one.
