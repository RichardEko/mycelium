...
- **CI now gates the 13-scenario integration cluster** ([`.github/workflows/cluster-suites.yml`](.github/workflows/cluster-suites.yml)).
  ...
  PRs, nightly, on merge to main, and on demand. Kept off trivial PRs (image build is heavy) and
  distinct from the 100-node scale suites (deliberately CI-excluded — single-host Docker-bridge
  ceiling). Wiring it exposed an integration flake (S13 convergence polls under CI load), so
  `run_scenario` gained the coop/fluid **retry-hardening** (`SCENARIO_ATTEMPTS`, default 2: a real
  regression fails all attempts, a transient miss passes on retry). Wiki `dev/examples.md` updated
  (and its stale `make test-integration` → `make test`). The **overlay** suite (`make test-overlay`)
  stays manual-only: gating it surfaced a deeper exact-once-delivery bug in `subscribe_log_group`
  (#149) that needs a focused fix, not a CI-blind one.
